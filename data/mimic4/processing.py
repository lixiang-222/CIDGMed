...
def create_str_token_mapping(df):
    ...
    dill.dump(obj={'diag_voc': diag_voc, 'med_voc': med_voc, 'pro_voc': pro_voc},
              file=open('./output/voc_final.pkl', 'wb'))
...
def create_patient_record(df, diag_voc, med_voc, pro_voc):
    ...
    dill.dump(obj=records, file=open('./output/records_final.pkl', 'wb'))
    return records
...
def get_ddi_matrix(records, med_voc, ddi_file):
    ...
    dill.dump(ehr_adj, open('./output/ehr_adj_final.pkl', 'wb'))
...
if __name__ == '__main__':
    ...
    cid_atc = './input/drug-atc.csv'
    ndc_rxnorm_file = './input/ndc2rxnorm_mapping.txt'
    ...
    # for procedure
    proc_pd = procedure_process(procedure_file)
    # proc_pd = filter_1000_most_pro(proc_pd)
